...
import {AdidasItemType} from "../Site";
type AdidasProps = {
    items: AdidasItemType[]
}

export const Adidas = ({items}: AdidasProps) => {
    ...
    return (
        <div>
            ...
            <div className={styles.wrapper}>
                {items ?
                    items.map((adidas, index) => (
                        <Link key={index} to={`/adidas/${adidas.idModel}`}>
                            <S.StyledImg src={adidas.picture}
                                         alt={adidas.model}
                                         width='200px'/>
                        </Link>
                    ))
